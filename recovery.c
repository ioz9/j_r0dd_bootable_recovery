...
choose_nandroid_file(const char *nandroid_folder)
{
    ...
    for (;;) {
        ...
        if (key == KEY_CLIQXT_BACK) {
            break;
        } else if ((key == KEY_CLIQXT_DOWN) && visible) {
            ++selected;
				...
				if (confirm == KEY_CLIQXT_HOME) {
					...
					for (;;) {
						...
						if (key == KEY_CLIQXT_HOME) {
							...
							break;
					        } else if ((key == KEY_CLIQXT_DOWN)) {
								swap=swap-32;
					        } else if ((key == KEY_CLIQXT_UP)) {
								swap=swap+32;
			                        }
						if (swap < 0) { swap=0; }
					} 
...
prompt_and_wait()
{
    ...
    static char* headers[] = { "MB501 Android Recovery",
			       "",
			       NULL };
